...
export class StoryErrorElement extends HTMLDivElement {
    ...
    constructor() {
        ...
        window.addEventListener('error', function showError (this: Window, er: ErrorEvent) {
            const {message, filename, lineno, colno} = er;
            ...
            if (self.#debug) {
                msg = JSON.stringify({message, filename, lineno, colno}, null, 2)
                    .replace(' ', '&nbsp;')
                    .replace('\n', '<br />');
            }else{
                msg = message;
